...
export function CartPageClient({ locale = DEFAULT_LOCALE }: { locale?: Locale }) {
  const dict = getDictionary(locale);
  const t = dict.cart;
  const tCheckout = dict.checkoutForm;
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto w-full max-w-2xl px-6 py-16 text-center">
        <h1 className="text-2xl font-bold text-ink">{t.empty}</h1>
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-12">
      <h1 className="text-2xl font-bold text-ink">
        {t.title} ({items.length} {t.products})
      </h1>
      ...
      <form onSubmit={handleCheckout} className="mt-8 space-y-4">
        <h2 className="text-lg font-semibold text-ink">{t.infoHeading}</h2>
        <div>
          <label className="block text-sm font-medium text-slate-700">{tCheckout.fullName}</label>
          <input
            required
            value={customerName}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700">{tCheckout.email}</label>
          <input
            required
            type="email"
            value={customerEmail}
        ...
        >
          {loading ? tCheckout.processing : tCheckout.createQr}
        </button>
      </form>
    </div>
  );
